...
type
  ...
  private
    ...
    class procedure MovingThroughSides(ASoObject, AWorld: TSoObject);
  public
...
procedure OnCollideAsteroid(ASender: TObject; AEvent: TObjectCollidedEventArgs);
begin
  TSoObject(ASender)[Sound].Val<TSoSound>.Play;
end;
...
class procedure TLogicAssets.MovingThroughSides(ASoObject, AWorld: TSoObject);
begin
  with ASoObject do begin
   if X < - Width then
     X := AWorld.Width + Width;
